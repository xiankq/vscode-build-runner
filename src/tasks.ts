...
export async function createTask(unique: string, uri: vsc.Uri, title: string, type: 'watch' | 'build') {
  ...
  const command = fvm
    ? `fvm dart run build_runner ${type} ${args}`
    : `dart run build_runner ${type} ${args}`;
